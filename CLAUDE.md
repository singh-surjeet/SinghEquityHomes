...
**GitHub Pages for hosting.** Chosen over the SFTP-to-third-party-hosting setup this repo started with, and over Cloudflare Pages, specifically because it's operable end-to-end from the GitHub Actions workflow and `gh` CLI without needing a separate hosting account connected through an external dashboard. The trade-off is the repo must be public, accepted for the initial deploy. The custom domain (registered via Cloudflare) is layered on top via DNS pointing at GitHub Pages — Cloudflare is only the registrar/DNS host here, not the site host.
